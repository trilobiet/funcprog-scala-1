...
trait DecoderInstances {
  ...
  Decoder.fromPartialFunction{
    case v : Json.Num if (v.value.isValidInt) => v.value.intValue
  }
  */
...
trait ContactsCodecs {
  ...
  implicit val contactsEncoder: Encoder[Contacts] =
    ObjectEncoder.field[List[Person]](name="people")
      .transform[Contacts]{case Contacts(persons) => persons}

  /** The corresponding decoder for `Contacts` */
  implicit val contactsDecoder: Decoder[Contacts] ={
    Decoder.field[List[Person]](name="people")
      .transform[Contacts]{case persons => Contacts(persons)}
  }
...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val maybeJsonInt = parseJson(""" 314 """)
    val maybeNotJsonInt = parseJson(""" 3.14 """)
    val maybeNotJsonBool = parseJson(""" "valse" """)
    val maybeJsonBool = parseJson(""" true """)
    val maybeJsonObj    = parseJson(""" { "name": "Alice", "age": 42 } """)
    val maybeJsonObj2   = parseJson(""" { "name": "Alice", "age": "42" } """)
    // Uncomment the following lines as you progress in the assignment
    println("314 Int?  " + maybeJsonInt.flatMap(_.decodeAs[Int]))
    println("3.14 Int?  " + maybeNotJsonInt.flatMap(_.decodeAs[Int]))
    println("foo Int?  " + maybeJsonString.flatMap(_.decodeAs[Int]))
    println("foo String?  " + maybeJsonString.flatMap(_.decodeAs[String]))
    println("valse Boolean?  " + maybeNotJsonBool.flatMap(_.decodeAs[Boolean]))
    println("true Boolean?  " + maybeJsonBool.flatMap(_.decodeAs[Boolean]))
