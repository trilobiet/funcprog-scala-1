...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    ...
    val q: List[(Block, List[Move])] = for {
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    neighbors.filterNot{ case (block,lst) => explored.contains(block) }
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList() // exit condition
    else {

      val nextInitial: LazyList[(Block, List[Move])] = for {
        (block, moves) <- initial
        neighbours = newNeighborsOnly(neighborsWithHistory(block, moves), explored)
        neighbour <- neighbours
      } yield neighbour

      val newBlocks = nextInitial.map{ case (block,_) => block }.toSet // 1st part = Block, 2nd part = List[Move]
      val newExplored = explored ++ newBlocks

      // grow initial
      // See https://github.com/scala/bug/issues/10370
      // initial ++ from(nextInitial, newExplored)
      initial #::: from(nextInitial, newExplored) // return value
    }
  }
  ...
  lazy val solution: List[Move] = {
    ...
    if (pathsToGoal.isEmpty) List.empty
      ...
      val paths = pathsToGoal.sortBy {
        case (_, moves: List[Move]) => moves.length
      }
      paths.head._2.reverse
    }
  }
}
